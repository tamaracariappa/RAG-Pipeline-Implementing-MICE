...
STRATEGIES = [
    {
        ...
        "flow": [
            ("Query", "#2D336B"),
            ("embed_query()", "#2D336B"),
            ("FAISS search · TEXT index", "#2D336B"),
            ("Top-k results", "#2D336B"),
        ],
    },
    {
        "key":    "B",
        ...
        "flow": [
            ("Query + FilterConfig", "#7886C7"),
            ("embed_query()", "#7886C7"),
            ("FAISS search · top_k × 20", "#7886C7"),
            ("Python metadata filter", "#7886C7"),
            ("Top-k filtered results", "#7886C7"),
        ],
    },
    {
        "key":    "B_prime",
        ...
        "flow": [
            ("Query + FilterConfig", "#A9B5DF"),
            ("embed_query()", "#A9B5DF"),
            ("FAISS search · expanded pool", "#A9B5DF"),
            ("Filter → semantic re-rank", "#A9B5DF"),
            ("Top-k results", "#A9B5DF"),
        ],
    },
    {
        "key":    "C",
        ...
        "flow": [
            ("Query", "#5C6BC0"),
            ('"work order description: " + query', "#5C6BC0"),
            ("embed_query()", "#5C6BC0"),
            ("FAISS search · MICE index", "#5C6BC0"),
            ("Top-k results", "#5C6BC0"),
        ],
    },
]
...
def _flow_diagram(steps: list) -> None:
    ...
    for i, (label, color) in enumerate(steps):
        parts.append(f"""
        <div style="background:#FFF2F2;border:1px solid {color};
...
def render():
    ...
    for col, s in zip(cols, STRATEGIES):
        with col:
            pros_html = "".join(
                f'<div style="font-size:0.73rem;color:#7886C7;margin:0.15rem 0;">'
            ...
            <div style="background:#FFFFFF;border:1px solid #A9B5DF;
                ...
                <div style="font-size:0.72rem;font-weight:600;
                            ...
                            color:#2D336B;margin:0.3rem 0 0.6rem;">{s['title']}</div>
                <div style="font-size:0.75rem;color:#7886C7;
    ...
    for s in STRATEGIES:
        with st.expander(
            ...
            with d1:
                st.markdown(f"""
                <div style="font-size:0.82rem;color:#2D336B;
                ...
                for p in s["pros"]:
                    st.markdown(
                        f'<div style="font-size:0.8rem;color:#7886C7;'
                ...
                st.markdown(f"""
                <div style="margin-top:0.8rem;font-size:0.75rem;color:#7886C7;">
                    <span style="color:#2D336B;">Index:</span> {s['index']}
                    &nbsp;·&nbsp;
                    <span style="color:#2D336B;">Filter:</span> {s['filter']}
                </div>""", unsafe_allow_html=True)
    ...
    with r1:
        ...
        <div style="background:#FFFFFF;border:1px solid #A9B5DF;
            ...
            <div style="font-size:0.8rem;color:#7886C7;line-height:1.6;">
    ...
    <div style="background:#FFFFFF;border:1px solid #A9B5DF;
        ...
        <div style="font-size:0.8rem;color:#7886C7;line-height:1.8;">
            ...
            <ul style="margin-top:0.4rem;padding-left:1.2rem;">
                <li>Equipment or type vocabulary detected → <strong style="color:#7886C7;">B′</strong></li>
                <li>Building ID pattern detected → <strong style="color:#7886C7;">B</strong></li>
                <li>No metadata signals → <strong style="color:#2D336B;">A</strong></li>
                <li>Explicit override → <strong style="color:#7886C7;">C</strong></li>
            </ul>
